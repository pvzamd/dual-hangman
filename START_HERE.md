...
|                     |                                                                                                                                                                                                                                               |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Completed**       | Phases 0–7: foundation → lobby → word setup → gameplay → game over + rematch → polish & resilience → **session scoring**. **Released as v1.0.0** (feature-complete).                                                                          |
| **Current phase**   | No phase in progress (v1.0.0 cut on `develop/v1`). See **Nice-to-Have / Future** in [docs/ROADMAP.md](docs/ROADMAP.md); deploy via [docs/MUST_FIX_BEFORE_DEPLOY.md](docs/MUST_FIX_BEFORE_DEPLOY.md).                                          |
| **Next task**       | Optional: keep playtesting, or pick a Nice-to-Have (e.g. dictionary validation, spectator mode). Best-of-N / persistence were intentionally left out.                                                                                         |
| **Working**         | Full game loop + polish: rooms, word setup, turn-based guessing, forfeit, game over (both words + defeat figure), rematch, session score, in-room chat, idle-room sweep, responsive layout, subtle animations, sound (mutable); 64 unit tests |
| **Not working yet** | Nothing planned outstanding. Best-of-N, persistence, accounts, leaderboards — out of scope by design (ADR-002).                                                                                                                               |
...
| 014 | Session score lives on the room (in-memory); +1 per round win, persists across rematches, resets when the room ends                                         |

---

## 5. Documentation Index

**Layout rule:** the repository root holds only the files that a human or tool meets _first_; everything else lives in `docs/`. These files stay at root, each for a specific reason:

| Root file           | Why it must (or should) be at the root                                                                                                                                                  |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `README.md`         | GitHub renders it on the repository landing page (convention).                                                                                                                          |
| `CHANGELOG.md`      | Conventional release history at the root; referenced by tags/releases.                                                                                                                  |
| `START_HERE.md`     | The universal human/AI entry point — must be obvious at the top, not buried in `docs/`.                                                                                                 |
| `CLAUDE.md`         | Claude Code auto-loads `./CLAUDE.md` from the repo root (tool convention).                                                                                                              |
| `CLAUDE_CONTEXT.md` | The volatile session snapshot; pairs with the auto-loaded `CLAUDE.md` and is step 3 of the workflow — co-locating the two AI-continuity files keeps cold-start resumption discoverable. |

Everything else — design, rules, decisions, progress, deployment, playtesting, and the release-review docs — is under `docs/`.
...
| [CHANGELOG.md](CHANGELOG.md)                                           | Release history (SemVer)                                           | Each tagged release                                         | **Source of truth: released versions**                                                                    |
...
| [docs/RELEASE_READINESS.md](docs/RELEASE_READINESS.md)                 | Pre-deploy review: debt, limitations, risks, doc audit             | Before a deploy, or after a broad review                    | Snapshot review (dated); re-run before shipping                                                           |
| [docs/MUST_FIX_BEFORE_DEPLOY.md](docs/MUST_FIX_BEFORE_DEPLOY.md)       | Blockers for a public deploy (config + DoS safeguard)              | A blocker is resolved or a new one is found                 | **Source of truth: deploy blockers**                                                                      |
| [docs/NICE_TO_HAVE_AFTER_DEPLOY.md](docs/NICE_TO_HAVE_AFTER_DEPLOY.md) | Non-blocking post-launch improvements                              | Items are done or added                                     | Backlog of optional work                                                                                  |
